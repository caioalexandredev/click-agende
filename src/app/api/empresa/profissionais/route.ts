...
export async function GET(request: NextRequest) {
  ...
  const springResponse = await springFetch("/funcionario/gerenciar", { headers });
  const payload = await parseSpringResponse(springResponse);

  if (!springResponse.ok) {
    return NextResponse.json(
      { message: getErrorMessage(payload, "Não foi possível carregar os profissionais.") },
...
export async function POST(request: NextRequest) {
  ...
  const springResponse = await springFetch("/funcionario/inserir", {
    method: "POST",
    headers,
  ...
  if (!springResponse.ok) {
    return NextResponse.json(
      { message: getErrorMessage(payload, "Não foi possível cadastrar o profissional.") },
      { status: springResponse.status },
    );
  }
