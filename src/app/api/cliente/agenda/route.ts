...
export async function GET(request: NextRequest) {
  ...
  const springResponse = await springFetch("/agenda/meus-agendamentos", { headers });
  const payload = await parseSpringResponse(springResponse);

  if (!springResponse.ok) {
    return NextResponse.json(
      { message: getErrorMessage(payload, "Não foi possível carregar seus agendamentos.") },
...
export async function POST(request: NextRequest) {
  ...
  const springResponse = await springFetch("/agenda/inserir", {
    method: "POST",
    headers,
  ...
  if (!springResponse.ok) {
    return NextResponse.json(
      { message: getErrorMessage(payload, "Não foi possível criar o agendamento.") },
      { status: springResponse.status },
    );
  }
